...
def callOneTrustProfiles(ot_api_key, ot_profile_url, pagenumber):
    payload = {'page': pagenumber, 'size': 50}  # according to one trust 50 is max you can get from one call.
...
def getOneTrustCollectionData():
    ...
    header_payload = {'APIKey': str(ot_api_key)}
    payload = {'size': 50}
    getresponse = requests.get(ot_collection_url, headers=header_payload, params=payload)
    ...
    try:
        ...
        for page in range(total_pages):
            df = callOneTrustCollectionPoint(
                ot_api_key, ot_collection_url, page)
